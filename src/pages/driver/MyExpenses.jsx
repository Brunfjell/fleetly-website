...
export default function MyExpenses() {
  ...
  const handleDeleteProof = async () => {
    if (!selectedExpense?.proof_url) return;
    setDeleting(true);
    try {
  ...
  const filteredAndPaginatedExpenses = useMemo(() => {
    let filtered = expenses.filter((e) => {
      ...
      const expDate = e.date ? new Date(e.date).toLocaleDateString("en-CA") : "";
      const matchesDate = !selectedDate || expDate === selectedDate;

      return matchesSearch && matchesType && matchesDate;
    });

    filtered.sort((a, b) =>
      amountOrder === "asc" ? a.amount - b.amount : b.amount - a.amount
    );

    const startIdx = (currentPage - 1) * itemsPerPage;
    return {
      filtered,
      paginated: filtered.slice(startIdx, startIdx + itemsPerPage),
    };
  }, [expenses, searchTerm, typeFilter, selectedDate, amountOrder, currentPage]);
